...
class LinearRegression:
    ...
    def predict(self, X):
     ...
     def computeCost(self, X, y, theta):
        ...
        """
        n = len(y)
        error = X @ theta - y  # Vectorized computation of the error
        cost = (1 / (2 * n)) * np.dot(error.T, error)  # Vectorized computation of the cost
        return cost.item()  # Return the scalar value

    def gradientDescent(self, X, y, theta):
        """
        Fits the model via vectorized gradient descent.
        Arguments:
            X is a n-by-d numpy matrix
            y is an n-dimensional numpy vector
            theta is a d-dimensional numpy vector
        ...
        for i in range(self.n_iter):
            error = X @ theta - y  # Vectorized computation of the error
            gradient = (1 / n) * X.T @ error  # Vectorized computation of the gradient
            theta = theta - self.alpha * gradient  # Vectorized update of theta
